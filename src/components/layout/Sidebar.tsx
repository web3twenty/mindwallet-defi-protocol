...
export const Sidebar = ({ activeId, onSelect }: { activeId: string; onSelect: (id: string) => void }) => {
  return (
    <aside className="w-72 bg-[#0C0C0C] border-r border-white/5 h-screen overflow-y-auto custom-scrollbar flex flex-col">
          ...
          <div key={idx} className="space-y-1">
            {group.items.map((item) => (
              <button
                ...
                className={cn(
                  "w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all group relative",
                  activeId === item.id 
