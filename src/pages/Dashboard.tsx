          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
             <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <h1 className="text-2xl md:text-3xl font-black font-mono tracking-tighter uppercase italic">Transaction Report</h1>
             ...
             <TransactionSection onAction={(type) => setActiveTab(type)} isFullPage />
          </div>
        );
      case 'profile':
        return <Profile />;
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0A] text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <Header 
          title={
            activeTab === 'elite' ? 'Elite Member' : 
            activeTab === 'profile' ? 'My Profile' : 
            activeTab === 'transactions' ? 'Transaction Report' :
            activeTab.replace('-', ' ')
          } 
      ...
      <AnimatePresence>
        ...
        {modal.type === 'apply-elite' && (
          <ApplyEliteModal 
            ...
            onClose={closeModal} 
            asset={modal.asset}
            onDeposit={() => openModal('deposit', modal.asset)}
          />
        )}
      </AnimatePresence>
    </div>
